...
func (r *StreamProfileRepository) Create(ctx context.Context, profile *models.StreamProfile) error {
	...
	result, err := r.db.ExecContext(ctx,
		`INSERT INTO stream_profiles (name, source_type, hwaccel, video_codec, container, use_custom_args, custom_args, command, args, is_default, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		profile.Name, profile.SourceType, profile.HWAccel, profile.VideoCodec, profile.Container, profile.UseCustomArgs, profile.CustomArgs, profile.Command, profile.Args, profile.IsDefault, now, now,
	)
	if err != nil {
...
func (r *StreamProfileRepository) GetByID(ctx context.Context, id int64) (*models.StreamProfile, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, name, source_type, hwaccel, video_codec, container, use_custom_args, custom_args, command, args, is_default, created_at, updated_at
		FROM stream_profiles WHERE id = ?`, id,
	).Scan(&profile.ID, &profile.Name, &profile.SourceType, &profile.HWAccel, &profile.VideoCodec, &profile.Container, &profile.UseCustomArgs, &profile.CustomArgs, &profile.Command, &profile.Args, &profile.IsDefault, &profile.CreatedAt, &profile.UpdatedAt)
	if err != nil {
...
func (r *StreamProfileRepository) List(ctx context.Context) ([]models.StreamProfile, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &p.SourceType, &p.HWAccel, &p.VideoCodec, &p.Container, &p.UseCustomArgs, &p.CustomArgs, &p.Command, &p.Args, &p.IsDefault, &p.CreatedAt, &p.UpdatedAt); err != nil {
...
func (r *StreamProfileRepository) Update(ctx context.Context, profile *models.StreamProfile) error {
	...
	_, err := r.db.ExecContext(ctx,
		`UPDATE stream_profiles SET name = ?, source_type = ?, hwaccel = ?, video_codec = ?, container = ?, use_custom_args = ?, custom_args = ?, command = ?, args = ?, is_default = ?, updated_at = ?
		WHERE id = ?`,
		profile.Name, profile.SourceType, profile.HWAccel, profile.VideoCodec, profile.Container, profile.UseCustomArgs, profile.CustomArgs, profile.Command, profile.Args, profile.IsDefault, now, profile.ID,
	)
	if err != nil {
