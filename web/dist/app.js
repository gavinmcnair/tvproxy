(function() {
  ...
  function flushJSErrors() {
    ...
    if (navigator.sendBeacon) {
      navigator.sendBeacon('/api/frontend-errors', new Blob([payload], { type: 'application/json' }));
    } else {
      fetch('/api/frontend-errors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: payload }).catch(function() {});
    }
  }
  ...
  console.error = function() {
    ...
    for (var i = 0; i < arguments.length; i++) {
      var a = arguments[i];
      parts.push(typeof a === 'object' ? JSON.stringify(a, null, 0) : String(a));
    }
    reportJSError('console.error: ' + parts.join(' '), '', 0, 0, null);
  };
  ...
  function showProgrammeModal(opts) {
    ...
    if (opts.vodStreamID && opts.mediaType) {
      var rematchIcon = document.createElement('button');
      rematchIcon.style.cssText = iconBtnStyle + 'font-size:15px;';
      rematchIcon.title = opts.tmdbID ? 'Rematch TMDB' : 'Match TMDB';
      rematchIcon.textContent = '\uD83D\uDD0D';
  ...
  function showRematchModal(streamID, mediaType, currentTitle, onDone) {
    var tmdbType = mediaType === 'series' ? 'tv' : 'movie';
    var overlay = document.createElement('div');
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:10000;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px);';
    overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
    document.addEventListener('keydown', function onKey(e) { if (e.key === 'Escape') { overlay.remove(); document.removeEventListener('keydown', onKey); } });
    ...
    header.style.cssText = 'padding:20px 24px;border-bottom:1px solid var(--border);';
    header.appendChild(Object.assign(document.createElement('div'), { style: 'font-size:18px;font-weight:700;color:var(--text-primary);margin-bottom:12px;', textContent: 'TMDB Match' }));
    ...
    searchInput.placeholder = 'Search TMDB...';
    searchInput.style.cssText = 'flex:1;padding:8px 12px;border-radius:8px;border:1px solid var(--border);background:var(--bg-input);color:var(--text-primary);font-size:14px;';
    var searchBtn = document.createElement('button');
    searchBtn.textContent = 'Search';
    searchBtn.style.cssText = 'padding:8px 16px;border-radius:8px;border:none;background:#3b82f6;color:#fff;cursor:pointer;font-size:14px;';
    ...
    resultsArea.style.cssText = 'overflow-y:auto;flex:1;padding:12px;';
    modal.appendChild(resultsArea);

    function doSearch() {
      resultsArea.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted)">Searching...</div>';
      api.get('/api/tmdb/search?query=' + encodeURIComponent(searchInput.value) + '&type=' + tmdbType).then(function(data) {
        resultsArea.innerHTML = '';
        if (!data || !data.results || data.results.length === 0) {
          resultsArea.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted)">No results found</div>';
          return;
        }
        data.results.slice(0, 10).forEach(function(r) {
          ...
          if (r.poster_path) {
            row.appendChild(Object.assign(document.createElement('img'), { src: '/api/tmdb/image?size=w92&path=' + encodeURIComponent(r.poster_path), style: 'width:46px;height:69px;object-fit:cover;border-radius:6px;flex-shrink:0;' }));
          } else {
            row.appendChild(Object.assign(document.createElement('div'), { style: 'width:46px;height:69px;background:rgba(255,255,255,0.05);border-radius:6px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:20px;color:var(--text-muted);', textContent: '\uD83C\uDFAC' }));
          }
          ...
          var year = '';
          if (r.release_date) year = r.release_date.substring(0, 4);
          else if (r.first_air_date) year = r.first_air_date.substring(0, 4);
          info.appendChild(Object.assign(document.createElement('div'), { style: 'font-size:14px;font-weight:600;color:var(--text-primary);', textContent: title + (year ? ' (' + year + ')' : '') }));
          if (r.overview) {
            var desc = document.createElement('div');
            desc.style.cssText = 'font-size:12px;color:var(--text-muted);margin-top:2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;line-height:1.4;';
            desc.textContent = r.overview;
          ...
          selectBtn.style.cssText = 'padding:6px 14px;border-radius:6px;border:none;background:#22c55e;color:#fff;cursor:pointer;font-size:13px;flex-shrink:0;';
        ...
        resultsArea.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted)">Search failed</div>';
      });
    }
  ...
  const pages = {
    ...
    'm3u-accounts': buildCrudPage({
      ...
      fields: [
        ...
        { key: 'username', label: 'Username', placeholder: 'Xtream username', showWhen: form => form.type === 'xtream' },
    ...
    'source-profiles': buildCrudPage({
      title: 'Source Stream Profiles',
      singular: 'Source Stream Profile',
      apiPath: '/api/source-profiles',
      create: true,
      update: true,
      columns: [
        { key: 'name', label: 'Name' },
      ...
      fields: [
        { key: 'name', label: 'Profile Name', placeholder: 'IPTV Provider (UK)' },
        { key: 'transport', label: 'Transport', type: 'select', options: [
          { value: 'http', label: 'HTTP/HTTPS' },
          { value: 'rtsp', label: 'RTSP (SAT>IP)' },
          { value: 'file', label: 'Local File' },
        ], default: 'http' },
        { key: 'probe_mode', label: 'Probe Mode', type: 'select', options: [
          { value: 'auto', label: 'Auto (probe each stream, cache result)' },
          { value: 'declared', label: 'Declared (use codec settings below, skip probe)' },
          { value: 'none', label: 'No Probe (let ffmpeg detect at playback)' },
        ], default: 'declared', help: 'Auto: probes once per stream, caches in probe.json. Declared: trusts the codec settings below. No Probe: fastest startup, ffmpeg handles everything.' },
        { key: 'video_codec', label: 'Expected Video Codec', type: 'select', options: [
          { value: '', label: 'Auto (probe each stream)' },
          { value: 'h264', label: 'H.264 / AVC' },
          { value: 'hevc', label: 'H.265 / HEVC' },
          { value: 'mpeg2video', label: 'MPEG-2 (DVB/SAT>IP)' },
          { value: 'av1', label: 'AV1' },
        ], default: 'h264', help: 'Auto: probe every stream individually (best for mixed content like tvproxy-streams). Set a codec to skip probing for consistent sources like IPTV.', showWhen: form => form.probe_mode !== 'none' },
        { key: 'audio_codec', label: 'Expected Audio Codec', type: 'select', options: [
          { value: '', label: 'Auto (probe each stream)' },
          { value: 'aac', label: 'AAC' },
          { value: 'aac_latm', label: 'AAC-LATM' },
          { value: 'mp2', label: 'MP2 (DVB)' },
          { value: 'ac3', label: 'AC3 / Dolby Digital' },
          { value: 'eac3', label: 'E-AC3 / Dolby Digital Plus' },
          { value: 'opus', label: 'Opus' },
        ], default: 'aac', help: 'Auto: probe every stream individually. Set a codec to skip probing for consistent sources.', showWhen: form => form.probe_mode !== 'none' },
        { key: 'container', label: 'Container Format', type: 'select', options: [
          { value: '', label: 'Auto (probe each stream)' },
          { value: 'mpegts', label: 'MPEG-TS (most IPTV/SAT>IP)' },
          { value: 'mp4', label: 'MP4' },
          { value: 'matroska', label: 'Matroska / MKV' },
          { value: 'avi', label: 'AVI' },
        ], default: 'mpegts', help: 'Auto: detect from probe. Set for consistent sources to skip probing.', showWhen: form => form.probe_mode !== 'none' },
        { key: 'deinterlace', label: 'Deinterlace', type: 'checkbox', default: false, help: 'Apply yadif deinterlace filter. Enable for SAT>IP and DVB sources which deliver interlaced content (1080i).' },
        { key: 'input_format', label: 'Input Format Hint', type: 'select', options: [
          { value: '', label: 'Auto-detect' },
          { value: 'mpegts', label: 'MPEG-TS' },
          { value: 'mp4', label: 'MP4' },
          { value: 'matroska', label: 'MKV' },
        ], default: '', help: 'Force input format (-f flag). Usually auto-detected, but needed for pipe input.' },
        { key: 'rtsp_transport', label: 'RTSP Transport', type: 'select', options: [
          { value: '', label: 'Default (UDP)' },
          { value: 'tcp', label: 'TCP (recommended for SAT>IP)' },
        ], default: '', showWhen: form => form.transport === 'rtsp', help: 'TCP eliminates UDP packet loss. Always use TCP for SAT>IP.' },
        { key: 'analyze_duration', label: 'Analyze Duration', type: 'number', default: 1000000, help: 'Microseconds. How long ffmpeg analyzes input to detect streams. SAT>IP needs 3000000, IPTV needs 1000000, radio can use 0.' },
        { key: 'probe_size', label: 'Probe Size', type: 'number', default: 1000000, help: 'Bytes. How much data to probe. SAT>IP MPEG-2 needs 10000000 for video dimensions. IPTV H264 needs 1000000.' },
        { key: 'max_delay', label: 'Max Delay', type: 'number', default: 0, help: 'Microseconds. Maximum demux delay. SAT>IP uses 500000. Leave 0 for default.' },
        { key: 'fflags', label: 'Format Flags', placeholder: '+genpts+discardcorrupt', default: '+genpts+discardcorrupt', help: 'ffmpeg format flags. +genpts generates missing timestamps. +discardcorrupt drops broken packets.' },
        { key: 'err_detect', label: 'Error Detection', placeholder: 'ignore_err', default: 'ignore_err', help: 'How ffmpeg handles input errors. ignore_err skips corrupt data instead of failing.' },
        { key: 'audio_resync', label: 'Audio Resync', type: 'checkbox', default: false, help: 'Add aresample=async=1000 to fix A/V sync drift. Essential for SAT>IP and live DVB streams.' },
        { key: 'fps_mode', label: 'FPS Mode', type: 'select', options: [
          { value: '', label: 'Default (variable)' },
          { value: 'cfr', label: 'CFR (constant frame rate)' },
        ], default: '', help: 'CFR eliminates stutter from dropped frames. Recommended for SAT>IP when transcoding.' },
      ],
    }),

    'stream-profiles': buildCrudPage({
      title: 'Client Stream Profiles',
      singular: 'Client Stream Profile',
    ...
    _mediaGrid: function(cfg) {
      ...
      var activeFilters = {};
      var searchInput, grid, countSpan, renderFn;
      ...
      function renderCard(item, parent) {
        ...
        card.dataset.sortName = cfg.getName(item);
        ...
        if (poster) {
          posterWrap.appendChild(h('img', { src: poster, style: 'width:100%;height:100%;object-fit:cover;' }));
        } else {
          posterWrap.appendChild(h('div', { style: 'padding:12px;text-align:center;color:#fff;font-size:14px;font-weight:600;text-shadow:0 1px 4px rgba(0,0,0,0.5);' }, cfg.getName(item)));
        }
        var badge = cfg.getOverlayBadge ? cfg.getOverlayBadge(item) : null;
        if (badge) {
          posterWrap.appendChild(h('div', { style: 'position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.7);color:#fff;font-size:10px;padding:3px 8px;border-radius:4px;font-weight:600;' }, badge));
        }
        card.appendChild(posterWrap);

        var info = h('div', { style: 'padding:10px 12px;' });
        info.appendChild(h('div', { style: 'font-size:13px;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;' }, cfg.getName(item)));
        ...
        if (genres && genres.length) {
          ...
          genres.slice(0, 2).forEach(function(g) { genreRow.appendChild(h('span', { style: 'font-size:9px;padding:1px 5px;border-radius:3px;background:rgba(59,130,246,0.15);color:#60a5fa;' }, g)); });
          info.appendChild(genreRow);
        }
        card.appendChild(info);

        card.onclick = function() { cfg.onCardClick(item); };
      ...
      return {
        ...
        buildFilterBar: function(container, pills, dropdowns, totalCount) {
          var filterBar = h('div', { style: 'display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:16px;' });

          searchInput = h('input', { type: 'text', placeholder: '\uD83D\uDD0D Search...', style: 'padding:5px 12px;border-radius:20px;border:1px solid var(--border);background:var(--bg-input);color:var(--text-primary);font-size:12px;width:180px;' });
          searchInput.oninput = function() { renderFn(); };
          filterBar.appendChild(searchInput);
          filterBar.appendChild(h('span', { style: 'width:1px;height:20px;background:var(--border);align-self:center;' }));

          pills.forEach(function(p) { makePill(p.label, p.key, filterBar, p.group); });

          if (dropdowns.length > 0) {
            filterBar.appendChild(h('span', { style: 'width:1px;height:20px;background:var(--border);align-self:center;' }));
            dropdowns.forEach(function(d) { makePillDropdown(d.label, d.options, d.keyPrefix, d.group, filterBar); });
          }
